...
public class Controller : MonoBehaviour
{
    ...
    [Header("Animation")]
    [Tooltip("The animator for the ship's turbine")]
    public Animator turbineAnimator; // Referência para o componente Animator da turbina
    ...
    [Tooltip("The shield game object attached to the player")]
    public GameObject shieldObject; // Arraste o seu shield_bubble para cá no Inspector

    [Header("Audio")]
    [Tooltip("The Sound of engine")]
    public AudioSource startEngineSound; // Arraste o Audio Source aqui no Inspector
    public AudioSource engineSound; // Arraste o Audio Source aqui no Inspector
    ...
    private void HandleInput()
    {
        ...
        // --- AQUI ESTÁ A LÓGICA DA TURBINA CORRIGIDA ---
        if (turbineAnimator != null)
        {
            // Verifica se o jogador está a carregar em qualquer direção (X ou Y)
            // .sqrMagnitude é mais eficiente que .magnitude para performance
            bool apertouQualquerTecla = moveInput.sqrMagnitude > 0.01f;

            // Isso envia o comando para o parâmetro "isMoving" do Animator
            turbineAnimator.SetBool("isMoving", apertouQualquerTecla);

            // Lógica do Som
            if (engineSound != null)
            {
                if (apertouQualquerTecla && !engineSound.isPlaying)
                {
                    startEngineSound.Play(); // Liga o som se estiver movendo e o som estiver parado
                    engineSound.Play(); // Liga o som se estiver movendo e o som estiver parado
                }
                else if (!apertouQualquerTecla && engineSound.isPlaying)
                {
                    engineSound.Stop(); // Desliga o som se parar de mover
//                    startEngineSound.Stop();
    ...
    /// <summary>
    /// Activates the shield and resets its lives/health
    /// </summary>
    public void GanharEscudo(int vidas)
    {
        if (shieldObject != null)
        {
            // Pegamos o novo script ShieldHealth
            ShieldController sHealth = shieldObject.GetComponent<ShieldController>();

            if (sHealth != null)
            {
                sHealth.ActivarEscudo();
            }
            else
            {
                // Caso você ainda não tenha trocado o script no objeto
    ...
    public void GanharBomba(int quantidade)
    {
        ...
        if (bombScript != null)
        {
            bombScript.AdicionarBomba(quantidade);
        }
    }
